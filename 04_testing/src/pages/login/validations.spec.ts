...
describe("pages/login/validations specs", () => {
  it("should return undefined validation result when passing wrong field", done => {
    ...
    // Act
    validations.validateField(vm, field, value).then(fieldValidationResult => {
      // Assert
  ...
  it("should return failed validation result when passing login field with empty value", done => {
    // Arrange
    const vm = {
      login: ""
    };
